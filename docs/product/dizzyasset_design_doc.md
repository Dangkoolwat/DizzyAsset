# DizzyAsset 개발 설계 문서 v1.4

- 기준 문서: DizzyAsset v1.4 PRD
- 플랫폼: macOS
...
v1.4는 v1.3의 구조를 유지하면서 구현 단계에서 혼동될 수 있는 영역을 명확히 한다.

- Quick Peek은 v1.0 포함 기능이지만 기본형으로 제한한다.
- Vision/Speech 전체 분석은 Provider 구조를 준비하되 v1.0 필수 런타임 범위로 보지 않는다.
- 무음 기능은 v1.0에서 감지 표시까지로 제한하고, 클린 파일 생성은 후속 기능으로 둔다.
- 외장 SSD, security-scoped bookmark, Final Cut Pro 드래그는 고위험 구현 영역으로 본다.
- 검색은 단순 유틸리티가 아니라 DizzyAsset의 핵심 탐색 엔진으로 설계한다.
...
검색은 단순 파일명 필터가 아니라 편집자 언어 기반 탐색 엔진으로 다룬다.
...
2. 태그/카테고리 join 검색
3. 한국어/영어 혼합 검색어 정규화
4. 편집자 언어 사전 기반 alias 확장
5. SQLite FTS5 기반 검색 인덱스
6. 최근 사용/최근 등록 기반 ranking 보정
...
Quick Peek은 v1.0에 포함하되, 초기 구현은 기본형으로 제한한다.
...
- 앱 활성/비활성 상태 전환
- 멀티 모니터 위치 처리
- Space 미리듣기와 선택 상태 동기화
- sandbox / accessibility 권한 이슈

설계 원칙:

- MainWindow 상태와 과도하게 결합하지 않는다.
- SearchService와 PreviewService는 재사용하되, Quick Peek 전용 ViewModel로 상태를 격리한다.
- 드래그 고급 기능은 후속 범위로 두고, v1.0은 검색/이동/미리듣기/닫기 흐름을 안정화한다.
...
Final Cut Pro 드래그는 workflow-critical 기능이므로 고위험 영역으로 본다.

검증해야 하는 항목:

- 실제 파일 URL 기반 drag item 전달
- 외장 SSD 파일 drag
- 앱 재실행 후 bookmark 복원 상태에서 drag
- 원본 파일 누락/권한 없음 상태에서 오류 표시
- Final Cut Pro 타임라인 또는 브라우저 영역으로 drop되는 동작

제약:

- 숨은 복사 기반 media management를 하지 않는다.
- 원본 파일을 자동 이동하지 않는다.
- 권한 복원 실패 시 DB 기록을 삭제하지 않는다.
...
v1.0에서는 Vision/Speech 전체 분석을 필수 런타임 기능으로 보지 않는다. 다만 Provider 구조와 분석 결과 저장 구조는 후속 확장을 위해 유지한다.
...
작업 공간 파일은 성격에 따라 보존/정리 정책을 분리한다.

- `Temp/` 파일은 안전한 범위에서 자동 정리할 수 있다.
- `Generated/Preview/` 파일은 재생성 가능한 캐시로 보고 용량 제한을 둘 수 있다.
- `Analysis/Waveforms/` 결과는 재생성 가능하지만 검색/미리보기 성능에 영향을 주므로 별도 정리 정책을 둔다.
- `Derived/` 파일은 원본에서 파생된 작업본이므로 사용자가 명시적으로 정리하기 전까지 보존한다.
- source asset이 사라진 derived asset은 즉시 삭제하지 않고 `orphaned`, `missing-source`, `recoverable` 상태로 표시한다.
...
- 원본 파일 경로
- 원본 파일이 포함된 폴더 또는 볼륨
- 외장 SSD/NAS 등 사용자가 선택한 저장소 위치
- 작업 공간 위치를 사용자가 변경한 경우 해당 workspace root
...
### 5.4 External Storage Recovery

DizzyAsset은 외장 SSD 기반 편집 리소스 관리를 핵심 시나리오로 본다.

대응해야 하는 상황:

- 외장 SSD 분리 후 재연결
- 볼륨 이름 변경
- 앱 재실행 후 bookmark 복원 실패
- security-scoped bookmark stale 상태
- 원본 파일은 존재하지만 권한이 없는 상태

원칙:

- 복원 실패를 조용히 무시하지 않는다.
- DB의 asset record를 즉시 삭제하지 않는다.
- 오른쪽 패널과 저장소 상태 영역에 offline/permission 상태를 표시한다.
- 사용자가 새 경로를 지정하면 기존 asset relation을 유지한 채 location을 갱신한다.
...
- 에셋 수가 증가하면 FTS5 도입
- 태그/카테고리 join 최적화 필요
- 한국어/영어 혼합 검색을 위해 normalized token을 저장한다.
- alias 검색은 편집자 언어 사전과 연결한다.
- 검색 결과는 filename, tag, category, recent usage를 기준으로 ranking할 수 있다.
...
다음 영역은 구현 전 별도 검증 또는 작은 prototype을 권장한다.

- Quick Peek: 전역 단축키, NSPanel focus, 미리보기 동기화
- Final Cut Pro Integration: 실제 파일 URL drag, sandbox 권한, 외장 SSD 파일 접근
- BookmarkStore / FileSystemAccess: stale bookmark, 볼륨 변경, 앱 재실행 복원
- WorkspaceManager: workspace 이동, 파생 파일 보존, orphan 처리
...
v1.4는 v1.3의 설계 방향과 문서 구조를 유지하면서, PRD v1.4에서 정리된 범위와 구현 리스크를 디자인 문서에 반영한 보강 버전이다.

### 반영 내용

- 기준 문서를 DizzyAsset v1.4 PRD로 갱신
- Quick Peek은 v1.0 포함 기능으로 유지하되 기본형 범위와 고위험 요소를 명시
- Final Cut Pro 드래그 연동을 workflow-critical 고위험 영역으로 명시
- Vision/Speech 전체 분석은 v1.0 필수 런타임 범위에서 제외하고 Provider 구조 준비로 정리
- Search 모듈에 검색 아키텍처 방향 추가
- 한국어/영어 혼합 검색, alias 검색, FTS5 확장, ranking 후보를 명시
- 작업 공간 생애주기 정책 추가
- Temp, Preview cache, Analysis output, Derived file의 보존/정리 기준 추가
- `workspace_items`, `usage_history` 테이블 후보 추가
- Security-scoped bookmark 저장/복원 상태를 세분화
- External Storage Recovery 섹션 추가
- 외장 SSD 재연결, 볼륨 이름 변경, stale bookmark, 권한 없음 상태 대응 명시
- Final Cut Pro Drag 오류에 외장 드라이브 offline, bookmark 복원 실패, drop 실패 추가
- 고위험 구현 영역과 별도 prototype 권장 영역 명시
